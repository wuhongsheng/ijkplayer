 ...
 * @param volume
 */
void ijkmp_set_filter_Info(IjkMediaPlayer *mp,const char *filterinfo)
{
    assert(mp);
    MPTRACE("%s(%f)\n", __func__, filterinfo);
    pthread_mutex_lock(&mp->mutex);
    av_log(mp->ffplayer, AV_LOG_DEBUG, "iijkmp_set_filter_drawtext  %s\n", filterinfo);
    ffp_set_filter_Info(mp->ffplayer, filterinfo);
    pthread_mutex_unlock(&mp->mutex);
    MPTRACE("%s()=void\n", __func__);
}
 ...
 * @param volume
 */
void ijkmp_set_filter_drawtext(IjkMediaPlayer *mp,const char *drawtext)
{
    assert(mp);
    MPTRACE("%s(%f)\n", __func__, drawtext);
    pthread_mutex_lock(&mp->mutex);
    av_log(mp->ffplayer, AV_LOG_DEBUG, "iijkmp_set_filter_drawtext  %s\n", drawtext);
    ffp_set_drawtext_content(mp->ffplayer, drawtext);
    pthread_mutex_unlock(&mp->mutex);
    MPTRACE("%s()=void\n", __func__);
}
